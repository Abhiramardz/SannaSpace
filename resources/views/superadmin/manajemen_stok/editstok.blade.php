...
<head>
    ...
    <title>Edit Bahan Baku - Sanna Space</title>
...
<div class="flex min-h-screen">
    ...
    <div class="flex-1 flex flex-col">
        @include('layout.superadmin.headersuperadmin', ['title' => 'Edit Bahan Baku'])

        <main class="p-4 md:p-8">
            <div class="max-w-2xl mx-auto">
                <div class="bg-white rounded-3xl p-8 shadow-sm border border-gray-100">
                    <form action="{{ route('superadmin.stok.update', $bahanBaku->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        
                        <div class="mb-6">
                            <label class="block text-sm font-bold text-gray-700 mb-2">Nama Bahan Baku</label>
                            <input type="text" name="name" required value="{{ old('name', $bahanBaku->name) }}"
                        ...
                        <div class="mb-6 grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="unit" required value="{{ old('unit', $bahanBaku->unit) }}" placeholder="Contoh: kg, gr, ml, pcs"
                                    class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500/20 focus:border-green-500 outline-none transition font-medium">
                                @error('unit')
                                    <p class="text-red-500 text-xs font-bold mt-2">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input type="number" step="any" name="stock" required value="{{ old('stock', floatval($bahanBaku->stock)) }}"
                        ...
                        <div class="mb-8">
                            ...
                            <input type="number" step="any" name="min_stock" required value="{{ old('min_stock', floatval($bahanBaku->min_stock)) }}"
                                class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500/20 focus:border-green-500 outline-none transition font-medium text-center">
